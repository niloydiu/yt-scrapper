  ...
  <body>
    <div class="container-main">
      <div class="hero-section fade-in">
        ...
        <p class="hero-subtitle">Extract complete metadata from ALL videos in any YouTube channel</p>
      </div>

      <div class="glass-card fade-in">
        <div class="input-group-premium">
          <label for="channelInput">
            <i class="bi bi-youtube"></i> YouTube Channel / Playlist URL
          </label>
          <input 
            id="channelInput" 
            type="text" 
        ...
        <button id="extractBtn" class="btn-extract">
          ...
          <span id="btnText">EXTRACT ALL VIDEOS</span>
        </button>
      ...
      <div id="downloadSection" class="download-section download-top" style="display: none;">
        ...
        <a href="#" id="downloadLink" class="btn-download">
          <i class="bi bi-cloud-download-fill"></i> DOWNLOAD JSON FILE
        </a>
      </div>
    ...
    <script>
      ...
      function createVideoCard(video) {
        ...
        card.onclick = () => window.open(video.youtubeVideoLink, '_blank');
      ...
      async function extract() {
        ...
        const filename = filenameInput.value.trim() || 'videos.json';
        
        if (!channel) {
          showAlert('Please enter a YouTube channel or playlist URL');
          input.focus();
          return;
        }

        if (!filename) {
          showAlert('Please enter an output filename (extension will be .json)');
          filenameInput.focus();
          return;
        }

        // Ensure server receives a .json filename, but allow user to omit extension
        const filenameToSend = filename.endsWith('.json') ? filename : filename + '.json';

        btn.disabled = true;
        btnText.innerHTML = '<i class="bi bi-arrow-repeat spinner"></i> EXTRACTING ALL VIDEOS...';
        statusBar.style.display = 'flex';
        statusText.textContent = 'Fetching complete video list...';
        results.innerHTML = '';
        ...
        try {
          ...
          // Use server-returned filename (ensures .json)
          downloadLink.href = `/download/${data.filename}`;
          
          btnText.innerHTML = '<i class="bi bi-check-circle-fill"></i> COMPLETE';
          setTimeout(() => {
            btn.disabled = false;
            btnText.innerHTML = '<i class="bi bi-rocket-takeoff-fill"></i> EXTRACT ALL VIDEOS';
          }, 3000);
...
def api_extract():
    """Extract ALL video metadata from YouTube channel/playlist."""
    ...
    try:
        log.info(f"Extracting ALL videos from: {channel}")
